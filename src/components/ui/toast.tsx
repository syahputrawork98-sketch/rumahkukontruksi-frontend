...
const ToastItem = ({ id, message, type, onClose }: ToastItemProps) => {
  return (
    <div
      className={cn(
        "flex items-center gap-3 rounded-[var(--radius-md)] px-4 py-3 shadow-[var(--shadow-2)]",
        "animate-fade-in",
        typeClasses[type]
      )}
    >
      <div className="shrink-0">{typeIcons[type]}</div>
      <p className="grow text-sm font-medium">{message}</p>
      <button
        type="button"
        onClick={() => onClose(id)}
...
export function ToastContainer() {
  ...
  return (
    <div className="pointer-events-auto fixed right-4 top-4 z-[9999] flex w-full max-w-sm flex-col gap-2">
      {toasts.map((toast) => (
